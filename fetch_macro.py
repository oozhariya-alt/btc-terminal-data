...
# Список Spot BTC ETF + название эмитента
ETF_NAMES = {
...
"""Farside возвращает числа в формате '245.0', '(15.0)' для отрицательных, '-' для нуля."""
if raw is None:
return 0.0
...
"""Farside даты: '29 Apr 2024'. Возвращаем ISO 'YYYY-MM-DD' или None."""
s = str(raw).strip()
...
def fetch_html() -> str:
"""Скачиваем HTML страницы Farside."""
...
# ─── Парсер таблицы Farside ────────────────────────────────────────────────
def parse_table(html: str):
"""
Возвращает (headers, daily_rows):
headers — список тикеров в порядке колонок (без 'Date' и 'Total')
daily_rows — список словарей {date, IBIT, FBTC, ...}
"""
soup = BeautifulSoup(html, "lxml")
table = soup.find("table")
if not table:
raise RuntimeError("Не нашёл <table> на странице Farside")

rows = table.find_all("tr")
if len(rows) < 2:
raise RuntimeError("В таблице меньше 2 строк — структура изменилась?")

# Парсим заголовок (первые 1-2 строки могут быть header'ами)
header_cells = []
for r in rows[:3]:
...
# ─── Расчёт агрегатов и сигнала ────────────────────────────────────────────
def compute_per_etf(headers, rows):
"""Для каждого ETF считаем today / 7d / 30d / ytd + history30d."""
if not rows:
return []
out = []
last = rows[-1]
...
"""Суммы по всем ETF: today / 7d / 30d / ytd."""
if not rows:
...
today_sum = sum_row(last)
sum_7d = sum(sum_row(r) for r in last_7)
sum_30d = sum(sum_row(r) for r in last_30)
sum_ytd = sum(sum_row(r) for r in ytd)
return {
...
"""Интерпретация и score [-1..+1] для подачи в нейро-предиктор."""
if not rows:
...
"history": rows[-90:],  # 90 дней для sparkline
},
}

OUT_FILE.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
print(f"[write] {OUT_FILE} OK · {len(rows)} строк · сегодня total={totals['today']} $M")

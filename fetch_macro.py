...
Farside за Cloudflare — простой requests/curl_cffi блокируются 403.
Используем Playwright (настоящий headless Chromium) — Cloudflare его пропускает.
В workflow перед запуском: playwright install --with-deps chromium.
"""
...
def fetch_html():
    """Скачиваем HTML через настоящий headless Chromium (Playwright). Обходит Cloudflare."""
    from playwright.sync_api import sync_playwright
    print("[fetch] Playwright headless Chromium")
    with sync_playwright() as p:
        ...
        try:
            ...
            # Ждём таблицу — Cloudflare часто делает интерстишал, потом редирект
            page.wait_for_selector("table", timeout=20000)
            html = page.content()
            print(f"[fetch] Playwright OK, {len(html)} chars")
        finally:
            browser.close()
    if not html or "<table" not in html.lower():
        raise RuntimeError("Получен HTML без таблицы — возможно интерстишал Cloudflare")
    return html
